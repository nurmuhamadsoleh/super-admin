...
const dummyData = [
  ...
  },
  {
    id: 5,
    dibuat: "Georgia",
    jabatan: "Sekertaris camat",
    judul: "Pengumuman",
    tanggal: new Date(),
  },
];

const BpdInformasiDesa = () => {
  ...
  return (
    <div className="informasi-desa">
      <SubTitle title="Informasi Desa" addButton={true} addFunction={onAddNews} />
      <div>
            ...
            <table id="zero_config" className="table  no-wrap  ">
              <thead>
                <tr>
                  <th className="pl-5">
                    <div>
                      <div className="pb-0 pb-md-2">Dibuat</div>
                    </div>
                  </th>

                  <th>
                    <div className="d-md-flex justify-content-between" onClick={(e) => handleChangeText(e)}>
                      <div>Jabatan</div>

                      <div className="d-none d-md-block">
                        <div style={{ fontSize: "10px" }}>
                          <i className={changeText ? "fas fa-chevron-up" : "fas fa-chevron-up text-secondary"}></i>
                          <br />
                          <i class={changeText ? "fas fa-chevron-down text-secondary" : "fas fa-chevron-down"}></i>
                        </div>
                      </div>
                    </div>
                  </th>
                  ...
                  <th onClick={(e) => handleChangeText(e)}>
                    ...
                    <div className="d-flex justify-content-between ">
                      ...
                      <div className="d-none d-md-block" onClick={(e) => handleChangeText(e)}>
                        <div style={{ fontSize: "10px" }}>
                          <i className={changeText ? "fas fa-chevron-up" : "fas fa-chevron-up text-secondary"}></i>
                          <br />
                          <i class={changeText ? "fas fa-chevron-down text-secondary" : "fas fa-chevron-down"}></i>
              ...
              <tbody>
                {response.length !== 0 ? (
                  response.slice(currentPage * pageSize, (currentPage + 1) * pageSize).map((data, i) => {
                    let number = i + 1;
                    return (
                      <tr key={data.id}>
                        <td className="">{data?.dibuat}</td>
                        <td className="">{data?.jabatan}</td>
                        <td className="">{data?.judul}</td>
                        <td className="">{data?.tanggal.toISOString()}</td>
                        <td className="d-flex">
                          <div
                  ...
                  <tr className="odd">
                    <td colspan="5" className="dataTables_empty text-center" valign="top">
          ...
          {pagination && (
            <Pagination>
              <PaginationItem disabled={currentPage <= 0}>
                <PaginationLink className="prev-next-buttons" onClick={(e) => handleClick(e, currentPage - 1)} href="#">
                  <i class="fas fa-chevron-left"></i>
                </PaginationLink>
              </PaginationItem>
              <div className="d-flex pagination-wrap pr-1 pl-1 mr-2 ml-2">
                {[...Array(pagesCount)].map((currentPageno, i) => (
                  <PaginationItem active={i === currentPage} key={i} className="outline-0">
                    <PaginationLink
...
export default BpdInformasiDesa;
